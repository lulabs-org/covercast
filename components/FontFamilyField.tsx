...
export function FontFamilyField({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (target.closest(`.${styles.fontDropdown}`)) return
      setOpen(false)
      setSearch('')
    }
  ...
  const displayLabel =
    ...
    (() => {
      const firstFont =
        value.match(/"([^"]+)"/)?.[1] ?? value.split(',')[0].trim().replace(/"/g, '')
      return firstFont
    })()
    ...
    <div className={styles.fontFamilyField}>
      <label className="field">
        ...
        <button
          ref={triggerRef}
          type="button"
          className={`${styles.fontTrigger}${open ? ` ${styles.fontTriggerOpen}` : ''}`}
      ...
      {open && (
        <div className={styles.fontDropdown} style={dropdownStyle}>
                ...
                {group.options.map((option) => {
                  ...
                  return (
                    <button
                      key={option.value}
                      type="button"
                      className={`${styles.fontOption}${isActive ? ` ${styles.fontOptionActive}` : ''}`}
          ...
          <div className={styles.fontDropdownFooter}>
            ...
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path
                  d="M7 1.5V9.5M4 4.5L7 1.5L10 4.5"
                  stroke="currentColor"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M1.5 9.5V11.5C1.5 12.05 1.95 12.5 2.5 12.5H11.5C12.05 12.5 12.5 12.05 12.5 11.5V9.5"
                  stroke="currentColor"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
