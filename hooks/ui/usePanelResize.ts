    ...
    if (stored) {
      ...
      return {
        leftPanel: Math.max(
          ...
          Math.min(LEFT_PANEL_MAX_WIDTH, parsed.leftPanel ?? LEFT_PANEL_DEFAULT_WIDTH),
        ),
        rightPanel: Math.max(
          RIGHT_PANEL_MIN_WIDTH,
          Math.min(RIGHT_PANEL_MAX_WIDTH, parsed.rightPanel ?? RIGHT_PANEL_DEFAULT_WIDTH),
        ),
      }
    }
  } catch {}
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    ...
    if (panel === 'left') {
      const newWidth = Math.max(
        LEFT_PANEL_MIN_WIDTH,
        Math.min(LEFT_PANEL_MAX_WIDTH, startWidth + delta),
      )
